...
# 2026-02-16 jgarcia  : Added singleton lock feature and benchmarking documentation.
...
unless (GetOptions( \%hOptions, "SOURCE_ODBC=s", "SOURCE_WAREHOUSE=s", "SOURCE_SCHEMA=s", "FLOW=s", "GET_PRODUCT", "STAGE=s", "VIEW_NAME=s", "MODFILE=s", "MAX_HOURS=s", "START_HOURS=s", "END_HOURS=s", "OUT_TRACE=s", "LOGFILE=s", "BENCHMARK_LOG=s", "PIPELINE_NAME=s", "PIPELINE_TYPE=s", "PROD_NOT_REGEXP=s", "LOCK_FILE=s")){
    print($usageMsg);
...
if (!defined($lockFile) || $lockFile eq "")
{
	...
	{
		$lockFile = "./log/n_getSnowflakeE142ModuleTrace.lock";
	}
}
...
# If benchmark logging requested, emit a JSONL line compatible with pipeline-service/models.py
# This section records execution stats for monitoring and performance evaluation
if (defined $hOptions{BENCHMARK_LOG} && length($hOptions{BENCHMARK_LOG}))
{
