...
unless ( GetOptions( \%hOptions, "SCHEMA=s", "FMT=s", "OUT=s", "LOGFILE=s", "ARCHIVE=s", "SRCLOT_LOOKUP", "USE_CHILD_PROD", "GENERATE_MESFLOT" ) && (@ARGV > 0))
{
    print "USAGE: $0 <fileName> --schema {schema} -fmt {dbtools fmt file} --out {output dir} --archive {archive dir} --log {log} --srclot_lookup --use_child_prod\n";
    exit(1);
    #pod2usage(3);
}
...
my $schema = $hOptions{SCHEMA};
my $fmt_file = $hOptions{FMT};
my $dir    = $hOptions{OUT};
my $basefile = "${dir}/${basename}";
my $events = {};
my @exts = qw(.cast .cmes .mes .lotGgen);
...
my ($busUnit, $facility, $parent_lot, $child_lot, $date_code, $parent_prod, $child_prod, $owner, $parent_owner, $trans, $transaction_date_time,
		$source_lot, $source_prod, $source_inv_item_type, $path, $event_type, $source_fab, $from_fab, $fab, $gen_event_name) = "";
...
while (<IN>){
    ...
    # 01-Dec-15 SAB New DBTools (v2.0) fixed issue with duplication of LOT2LOTGEN entries due to duplicate events being loaded.
    # Can't rely on transaction date in MES data matching split date in PSoft, so check for existing gen event data based on event_name
    my $sth=$mydb->prepare("SELECT * FROM ".$schema.".LOT_GEVENT ev WHERE ev.EVENT_NAME = ?");
    $sth->execute($genevtName);			
    if(my $recs=$sth->fetchrow_hashref()) 
    {
        INFO("Event ".$genevtName." exists in ".$schema.".LOT_GEVENT, skipping");
        $foundMeta = 1;
    }
    else
    {
        # Check for duplicate entry in this file
...
if ($ext =~ /\.lotGgen/i) {
	$newfile_ok = "${basefile}.lotGmesgen";
	unlink ${newfile_ok} if -e ${newfile_ok};
} else {
	$newfile_ok = "${basefile}.mesgen";
	unlink ${newfile_ok} if -e ${newfile_ok};
}
...
if ( $rows_written > 0 )
{
		INFO("Running DBTOOLS to load $newfile_ok");
    system("$ENV{DPBIN}/dbtools -n 29 -db $schema -fmt $fmt_file -infile $newfile_ok");
    my $rc=$?;
    if ( $rc == 0 )
    {
