#!/usr/bin/env perl_db
# SVN $Id: fcs_eagle_log_IFF.pl 2581 2020-10-06 01:38:06Z dpower $
...
=head1 CHANGES
 ...
 2018-Dec-18 eric	: modified rel env's to cater ONRMS data
 2019-Jan-04 eric	: added aseft environment
 2019-Jan-17 eric	: remove _FT from the lotid (cpft env)
 2019-May-30 eric	: unzip file if file is zipped
 2020-Jun-30 jgarcia : list out arguments for dataItems in formatter instance variable. to include touchdown_num.
 2020-Sep-30 karen       : added support to fork and qde output (IFF)/files to designated location
 2021/03/25 jgarcia : get new Facility name from $DPSCRIPT/facilityMapping.ini and compress IFF file to gzip after.
 2021/04/20 jgarcia : modified to not hardcode testflow code reference file location. pass in to addTestFlowtoTP instead to load_testflow_ref.
 2021/04/20 jgarica : modified to support meft facility.
 2021/09/16 gmllego : list out arguments for dataItems to include ecid.
 2022/03/09 jgarcia : added support for adding Tester Hostname into the program name for SZ FT AC and DC.
 2022/03/09 jgarcia : add support to new ECID location for Final Test, always generate a sequence number as a partid for Final Test.
 2022/12/14 jgarcia : program name length update to 235 excluding suffix, session type.
 2023/06/15 jgarcia : modified to get SourceLot-(CZ4 fab)value from On_Lot refdb.					:
 2023/08/16 eric	: removed setting default wmc if wmc is not found. 
 2023/09/28 eric	: perform BK lot lookups by stripping characters
 2023/12/21 jgarcia : specific to SZ FT, use record 145 with pip "|" in the value as Loadboard and Dutboard.
					: loadboard is the left from the pipe and Dutboard is the right from the pipe.
					: ex, 145,"this is loadboard | this dutboard "
 2024/09/20 eric	: Change minimum device count for xFSC Eagle Translator
 2025/09/01 gmllego     : Added new Lot lookup rule for BK Probe Lot starts with 'L', ends with 'A' (more than 5 characters long).
...
unless (
    GetOptions(
        \%hOptions,  "OUT=s", "FORK=s", "SITE=s", "LOC=s", "FACILITYFILE=s", "FINALLOT", "V", "CONFIG=s",
         "RELLOT", "NOLOOKUP", "LOGFILE=s", "DEBUG", "TRACE", "METASTRIP", "PPLOG", "QDE"
    )
    )
{
...
unless ( grep { $_ eq $site } qw/cpft szrel szft pmft merel meft aic_my_ft atec_ph_ft etrend_tw_ft hana_th_ft gtk_tw_ft utac_th_ft mtsort pmsort casort cpsort szsort slsort mesort bksort amkor_tw_csp isti_tw_csp isti_tw_sort_mosaic_bk its_tw_ft gtk_tw_sort vgrd_tw_sort cprel its_tw_ft aseft/ ) {
...
if ($infile =~ /\.gz$/) {
	...
	$output =~ s/\.zip$//;
	unzip $infile => $output or die "unzip failed: $UnzipError\n";
	INFO ("unzipped file = $output");
}
...
my $parser = PDF::Parser::Eagle->new;
...
given ($site) {
   ...
   when ('its_tw_ft') {
    ...
    when ('szft') {
        # ######################################################################
        # SUZHOU EAGLE LOTID TRIMMING LOGIC
        # ----------------------------------------------------------------------
        # Requirement: Trim "R/r" and "REJ" suffixes to retrieve base Lot ID.
        # 1. REJ/RETEST Suffix: Remove keyword and everything after it.
        #    e.g., "X003010349REJ1" -> "X003010349"
        # 2. R/r Suffix: Remove if it's the last character of the base Lot ID.
        #    - Case-sensitive: Only remove 'r' if it was lowercase in the log.
        #    - Robustness: Also removes 'R' if uppercase in log.
        # 3. Safety: Truncate to 10 chars if still longer (Suzhou standard).
        # 4. Lookup: Use trimmed ID for genealogy check; fail to sandbox if meta not found.
        # ######################################################################
        my $lot = $header->LOT;
        # Get raw lot from misc if available to handle case-sensitivity for 'r'
        my @item120 = (exists $misc->{120}) ? @{ $misc->{120} } : ();
        my $raw_lot = (scalar @item120 > 5) ? $item120[5] : $lot;

        $lot =~ s/^AO/A0/;

        # 1. Handle REJ/RETEST suffix using regex to find the base Lot ID
        if ($lot =~ /REJ|RETEST/i) {
            $header->INDEX2("O");
            $reglim_flg = "N";
            # Remove REJ/RETEST and everything following it to get the base ID
            $lot =~ s/(REJ|RETEST).*$//i;
        }

        # 2. Handle 'r/R' suffix on the base Lot ID
        # Determine the raw base lot by matching the same pattern
        my $raw_base = $raw_lot;
        $raw_base =~ s/(REJ|RETEST).*$//i;

        if ($raw_base =~ /r$/) {
            # Trim R (since $lot is uppercased) if original ended in lowercase 'r'
            $lot =~ s/R$//i;
        }
        elsif ($lot =~ /R$/) {
            # Also trim uppercase 'R' if it's there
            $lot =~ s/R$//i;
        }

        # 3. Final Safety: If Suzhou standard is still 10 chars, trim any remaining 
        # extra characters that aren't 'r' or 'REJ'
        if ( length($lot) > 10 ) {
            $lot = substr($lot, 0, 10);
        }
		...
		INFO("TEST===>".@item);
		  ...
		  if($addr =~ /.*\|.*/) {
			INFO("BOARDs=$addr");
			my ($loadBoard, $dutBoard) = split(/\s?\|\s?/, $addr);
			INFO("BOARD2=$loadBoard||$dutBoard");
			$header->EQUIP4_ID(trim($loadBoard));
			$header->INDEX1(trim($dutBoard));
		  }
      	}
        my @item = @{ $misc->{120} } if ( exists $misc->{120} );
        my $stationId = trim($item[8]);
        INFO("Station ID=>>$stationId<<");
        if($stationId =~ /.+AC|.+DC/i) {
          INFO("Add Stationd ID in Program Name..");
          $header->PROGRAM($header->PROGRAM . "::" .$stationId);
          INFO("PRG=$header->{PROGRAM}");
      }
      #added revsion to the program name
      my $rev = $header->{REVISION};
      $header->PROGRAM($header->PROGRAM . "::" .$rev);
        }
	...
	if ($header->LOT =~ /\_FT$/i) {  #remove _FT (ie P001208018_FT)
...
my $testFlowCodeRefFile = "${dirname}/testFlow.ref";
my $testFlowCode = &addTestFlowtoTP($model,$testMode,$site, $testFlowCodeRefFile);
...
# change the final program length without test flow code and suffix to 236 from 45. program.ppid field is now a varchar2(255)
if ( grep { $_ eq $site } (qw/gtk_tw_ft hana_th_ft atec_ph_ft isti_tw_csp its_tw_ft amkor_tw_csp etrend_tw_ft utac_th_ft aic_my_ft gtk_tw_sort vgrd_tw_sort/) ) {
	...
	# Check Program length for > 235.  Truncate and send to sandbox.
	if ( length($program) + length($testFlowCode) > 235 )
	{
	    ...
	    $wr->forSBox(1);
	    $reglim_flg = "N";
	    $program = substr($program, 0, 234-length($testFlowCode)); # Leave enough room for testFlowCode
	...
	if ($hOptions{LOC} eq 'BK' && $hOptions{SITE} eq 'bksort' && $hOptions{METASTRIP}) {
		...
		unless ( $header->populateMeta ){
            ...
            # New rule: Lot starts with 'L', ends with 'A', and is longer than 5 characters
                        elsif ($origLot =~ /^L/ && $origLot =~ /A$/ && length($origLot) > 5) {
                               INFO("New Rule: Lot starts with 'L', ends with 'A', and is longer than 5 characters.");
                               my $newLot = substr($origLot, 0, -1);  # Remove last character
                                  $header->LOT($newLot); 
		...
		unless ( $header->populateMeta ){
			$wr->noMeta(1);
			$reglim_flg = "N";
			#INFO("FAB=$header->{FAB}||SITE=$site");
			# if($site eq "isti_tw_sort_mosaic_bk" && $header->{FAB} =~ /^CZ4:.*/i) {
			# 	my $lot = $header->{LOT};
			# 	my $url = "${ertUrl}${lot}";
			# 	INFO("FINAL URL=$url");
...
if ( $stats->{deviceCount} <= 1 && !($hOptions{RELLOT}))
{
