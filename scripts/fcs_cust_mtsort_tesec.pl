#!/usr/bin/env perl_db
# 2015-Dec-29 Eric	: Initial release
#
...
my $tfc_yld_dir = "${dir}/TFC_YLD";
my $dta_dir = "${dir}/DTA";
my $status;

if ($infile =~ /\.YLD/i || $infile =~ /\.TFC/i){
	system "/bin/mv -f \'${infile}\' ${tfc_yld_dir}/${move_file}";
	$status = (-e "${tfc_yld_dir}/${move_file}") ? "Successful" : "Failed";
}	
elsif ($infile =~ /\.DTA/i){
	system "/bin/mv -f \'${infile}\' ${dta_dir}/${move_file}";
	$status = (-e "${dta_dir}/${move_file}") ? "Successful" : "Failed";
}
