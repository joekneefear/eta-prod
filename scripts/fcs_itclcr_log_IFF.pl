#!/usr/bin/env perl_db
# SVN $Id: fcs_itclcr_log_IFF.pl 2589 2020-10-06 06:02:40Z dpower $

=pod

=head1 SYNOPSIS

  fcs_itclcr_log_IFF.pl <Input flie name>
...
B<This script> will read itclcr log file and generate IFF file for dbascii
...
=head1 CHANGES
 ...
 2015/05/01 jgarcia: modified to check for wmap reference data only if it is not FT LOT, to avoid iff being generated to stage_sanbox out folder and cannot be loaded to Prod even if Meta data found in FT reference table.
 ...
 2016-Mar-16 gilbert: atec_ph_ft_itclcr: capture loadborad and trim lot.
 ...
 2016-Apr-26 eric	: added new site me_rel for REL loading
 2016-May-10 eric	: append orig program name to program in me_rel.
 2016-May-11 eric	: me_rel :check stress duration/temp is in rage, removed attributes in program name, 
 ...
 2016-May-16 gilbert    : added cprel_itclcr
 ...
 2016-Nov-25 GilbertM   : Copy of fcs_eagle_log_IFF.pl
 2017-Mar-21 eric	: assign source lot as wafer name
 ...
 2020-Oct-06 karen 	: added support to fork output (IFF)/files to designated location
...
unless ( grep { $_ eq $site } qw/cpft szrel szft pmft me_rel meft aic_my_ft atec_ph_ft etrend_tw_ft hana_th_ft gtk_tw_ft utac_th_ft mtsort pmsort cpsort szsort slsort mesort bksort amkor_tw_csp isti_tw_csp gtk_tw_sort vgrd_tw_sort cprel/ ) {
...
# wsanopao: Set Raw File ==> infile and Environment ==> $site.itclcr_
$pplogger->setRawFile($infile);
$pplogger->setEnv($site,'itclcr');
...
my $parser = PDF::Parser::ITCLCR->new;

my ($model, $sbox_flg) = $parser->readFile( $output );
...
# G. Miole   - or file contains 10 or less results.
#if ( $stats->{deviceCount} <= 10 && !($hOptions{RELLOT}))
#{
#   dpExit( 1, "Too few devices to create IFF (".$stats->{deviceCount}.")");
#}
...
given ($site) {
    ...
    when ('szrel') {
        ...
        my $qpnum = substr $item[1], 0, 8;
        my $devchar = substr $item[1], 8, 1;
        my $lotchar = substr $item[1], 9, 1;
        my $strname = $item[2];
        my $strdur = $item[3];
        my $temp = $item[4];
        my $dtype = $item[5];
	...
	my @item    = split /\_/, $base_fn; 
	my $qpnum   = substr $item[0], 0, 8;
	my $devchar = substr $item[0], -2, 1;
	my $lotchar = substr $item[0], -1, 1;
	my $strname = $item[1];
	my $strdur  = $item[2];
	my $temp    = $item[3];
	my $dtype   = $item[4];
...
if ( grep { $_ eq $site } (qw/gtk_tw_ft hana_th_ft atec_ph_ft isti_tw_csp amkor_tw_csp etrend_tw_ft utac_th_ft aic_my_ft gtk_tw_sort vgrd_tw_sort/) ) {
