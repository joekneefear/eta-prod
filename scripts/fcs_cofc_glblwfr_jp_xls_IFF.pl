#!/usr/bin/env perl_db
# SVN $Id: fcs_cofc_gblwfr_jp_xls_IFF.pl 
=pod

=head1 SYNOPSIS

      fcs_cofc_glblwfr_jp_xls_IFF.pl <Input flie name>
...
(C) ON Semiconductor Inc. 2017 All rights reserved.
...
my $parser = PDF::Parser::COFC_GBLWFR_JP_xls->new;
...
foreach my $itemno ( sort keys %$td) {
	...
	$header->LOT($itemno."_".$$td{$itemno}{DELYNUM});
	...
	$header->PRODUCT($$td{$itemno}{PARTNO});
	$header->EQUIP2_ID($$td{$itemno}{DELYNUM});
	$header->EQUIP5_ID($$td{$itemno}{CUSTOMER});
	$header->EQUIP6_ID($facility);

    	# truncate ppid
	my $program = $header->PROGRAM;
	my $partno = "_".$$td{$itemno}{PARTNO};
    	if (length($program) > 35) {
        	INFO("PROGRAM NAME \"".$program.$partno."\" will be truncated to 35 characters.  Sending to sandbox.");
        	$wr->forSBox(1);
        	$program = substr($program, 1, 35-length($partno));
    	}
    	$header->PROGRAM_CLASS(9);
	$header->PROGRAM($program.$partno);
	$model->updateProgram;
