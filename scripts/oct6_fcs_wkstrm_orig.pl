...
#
#      $Id: fcs_wkstrm.pl 1697 2016-06-27 14:41:58Z dpower $
      my($sVersionId) = ( split(' ', '$Revision: 1697 $') )[1];
	  my($VersionAndDate) = "
...
#my $readWksm = "/home/dpower/project/work/jacky/wkstrm/read_wksm.pl";
	...
	while($files->[$index]){
		...
		if(defined($convert) && $convert){
			...
			if($basename =~ /lot_v21$/){
				...
				GenerateCSVFile($basename,$ext,"eatr",1);
			}
		}
...
sub OutputEhist{
	...
	MakeSubDir("ehist");
	my $outputfile=$hOptions{out}."/ehist/".$ehistfilename."\.ehist";
		...
		foreach $lehash (@lelines){
			...
			my $newline = join(",",(FormatField(${$lehash}{"lot_id"}), FormatField(${$lehash}{"facility"}), FormatField(${$lehash}{"transaction"})
						...
						,FormatField(${$lehash}{"rework_cat"}),FormatField(${$lehash}{"hold_flag"}),FormatField(${$lehash}{"hold_cat"})
									...
									,FormatField(${$lehash}{"lot_quantity_new"}),FormatField(${$lehash}{"loss_quantity"})                                                                        ,FormatField(${$lehash}{"sourcelot"}),FormatField(${$lehash}{"lotclass"})))."\n";
...
#			my($sourcelot,$lotowner,$lotclass,$product) = GetMetaByLot($lot);
			...
			for($i=1;$i<=$#vs+1;$i++){
				...
				$newline=FormatField($datetime)
						...
						."\cK".FormatField($lhistfsinfo{$lhistfskey}{'sourcelot'})
				...
				my $fname=$preoutputfilename."_".$prod;
				...
				if(defined($missingLot{$lot}) || defined($missingProduct{$prod})){
					if(defined($erroutput{$fname})){
						...
						$erroutput{$fname} .= "<DATA>\n";
						$erroutput{$fname} .=  "date_time,Lot_id,fab,Sequence_number,Condition1,Condition2,Condition3,Step,entity_type,equip_id,product_id,unit_id,site,parameter_set_id,parameter_set_version,parameter name,exceed_limit,test_flag,parm_grp_1,parm_grp_2,parm_grp_3,format_flag,result,low_lim,high_lim,recipe,stage,lot class,operator,SOURCE_LOT,LOT_CLASS\n";
					}
#					INFO("ERROUTOUT (".defined($missingLot{$lot}).")".$lot." (".defined($missingProduct{$prod}).")".$prod);
					...
					if(defined($output{$fname})){
						...
						$output{$fname} .= "<DATA>\n";
						$output{$fname} .=  "date_time,Lot_id,fab,Sequence_number,Condition1,Condition2,Condition3,Step,entity_type,equip_id,product_id,unit_id,site,parameter_set_id,parameter_set_version,parameter name,exceed_limit,test_flag,parm_grp_1,parm_grp_2,parm_grp_3,format_flag,result,low_lim,high_lim,recipe,stage,lot class,operator,SOURCE_LOT,LOT_CLASS\n";
					}
					$output{$fname} .= $newline."\n";
				}
	...
	for($i=1;$i<=$#vs+1;$i++){
		...
		if(!defined($offoutput{$phistfilenname})){
			$offoutput{$phistfilenname} = join("|",("date_time","fab","entity_type","entity_id","unit_id","site","parameter_set_id","parameter_set_version"
													...
													,"parameter_grp_2","parameter_grp_3","format_flag","result","low_lim","high_lim"))."\n";
		}
		my $offline = join("|",(FormatField($phistinfo{$holder}{'date_time'}),FormatField($phistinfo{$holder}{'facility'}),FormatField($entinfo{$entkey})
							,FormatField($phistinfo{$holder}{'type_id'}),FormatField($unitid),FormatField($currentsite),FormatField($phistinfo{$holder}{'parameter_set_id'})
							,FormatField($phistinfo{$holder}{'parameter_set_version'}),FormatField($phistinfo{$holder}{'parameter_name'}),FormatField($phistinfo{$holder}{'exceed_limit_flag'}),FormatField($phistinfo{$holder}{'test_data_flag_1'})
							,FormatField($parag1), FormatField($parag2), FormatField($parag3), FormatField($phistinfo{$holder}{'format_flag'})
							,FormatField($vs[$i-1]), FormatField($phistinfo{$holder}{'low_lim'}), FormatField($phistinfo{$holder}{'high_lim'})))."\n";
...
sub GetMetaByLot{
	...
	if($hOptions{FINALLOT}){
		return ($lot,$hash->{lot_owner},$hash->{lot_class},$hash->{product});
	}else{
		return ($hash->{source_lot},$hash->{lot_owner},$hash->{lot_class},$hash->{product});
	}
}
##############################################################################
...
#		my($sourcelot,$lotowner,$lotclass,$product) = GetMetaByLot($lot);
		...
		my $newline=FormatField($lhistinfo{$holder}{"lot_id"}).",".FormatField($lotinfo{$lhistinfo{$holder}{"lot_id"}})
					...
					.",".FormatField($entinfo{$entkey}).",".FormatField($lhistinfo{$holder}{"sourcelot"})
		...
		if($#work >= 1){
			...
			if($lot =~ /^\s*$/){
				...
				if(defined($missingLot{$lot})){
					...
					$erroutput{$newbasename} .= $new_line."\t".FormatField($sourcelot)."\t".FormatField($lotclass)."\n";
				}else{
			                if ( $lot ne $last_lot ) 
					{
					...
					$output{$newbasename} .= $new_line."\t".FormatField($sourcelot)."\t".FormatField($lotclass)."\n";
				}
				$last_lot = $lot;
			}
			my $attr_name = $work[2];
	...
	foreach $line(@lines){
		...
		if (not $iGotHeader)
		{
			...
			if(defined($missingLot{$lotid})){
				...
				$erroutput{$basename} .= $line.",".FormatField($sourcelot).",".FormatField($lotowner)."\n";
			}else{
				if(defined($output{$basename})){
				}else{
					$output{$basename} .= $headerline.",\"SOURCE_LOT\",\"LOT_OWNER\"";
				}
				$output{$basename} .= $line.",".FormatField($sourcelot).",".FormatField($lotowner)."\n";
			}
...
sub GenerateCSVFile{
	...
	if($ifsubdir){
		...
		$outputfile=$hOptions{out}."/".$basename."\.".$ext."\.".$newext;
	}
...
=cut
		...
		if (not $iGotHeader)
		{
			...
			my($sourcelot,$lotowner,$lotclass,$productid) = GetMetaByLot($lotid,$in_productid);
...
#					WARN("duplicate lotid and facility line in fabsite lhist file--key=".$key." line=".$line);
					...
					$lhistfsinfo{$key}={"lot_id"=>$lotid,"sourcelot"=>$sourcelot,"lotclass"=>$lotclass,"facility"=>$facility,"operation"=>$operation, "product_id"=>$productid
...
#					26-Jun-15 SAB Adding an array of lot(s) to indicate every lot processed through this tool at this time
					...
					$lhistfsinfo{$key}={"lot_id"=>"##MULTI##","facility"=>$facility,"operation"=>$operation, "product_id"=>$productid
...
sub ProcessAndOutputEntFile{
	...
	MakeSubDir("ent");
	my $outputfile=$hOptions{out}."/ent/".$basename."\.".$ext."\.ent";
	my $fh=undef;
