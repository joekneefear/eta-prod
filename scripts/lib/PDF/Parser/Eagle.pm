# SVN $Id: Eagle.pm 2570 2020-09-01 13:53:57Z dpower $
#
# CHANGES
...
# 2021-Sep-16 gmllego : modified to add FullBarCode as testnames and put value in die as ecid.
# 2022-Mar-09 jgarcia : add support to new ECID location for Final Test, always generate a sequence number as a partid for Final Test.
# 2022-Dec-14 jgarcia : source out ECID from record 5 instead from record 130. removed ECID as one of the parameters.
# 2024-Aug-24 sboothby : Get ECID key from the first 11 record.  Eagle LOG files can contain multiple instances of the 11 record, 
#                        SZ FT seems to always use the first to identify the full barcode.
 

package PDF::Parser::Eagle;
use strict;
use PDF::DpData;
use PDF::DpLoad;
use PDF::Log;
...
sub readFile {
    ...
    while (<INFILE>) {
        ...
        #2022-Dev-14 -jgarcia : 2DID or ecid from record 5 but not be loaded as one of the parameters. will be loaded as ECID KeyIndex instead.
        #2024-Aug-07 -sboothby : Only take the key from the first 11 record in the file.  If the key is already set, do not change it.
        if ( $item[0] eq "11" && $site =~ /szft/i && !defined($record5_key)) {
              $record5_key = $item[1];
        }
        if ( ($item[0] eq "5" && $item[1] == $record5_key) && $site =~ /szft/i ) {
        ...
        if ( $item[0] eq "130" ) {
            ...
            if ( $die_cnt == 1 ) {
                ...
                $prevPartTestTime =
                  $currentPartTestTimeSec - $prevPartTestTimeSec;
        ...
        if ( $item[0] eq "50" ) {
            ...
            $rec50_flg = "Y";
            $sbr{ $item[1] } = { name => trim( $item[4] ) };
        }
        if ( $item[0] eq "60" ) {

            $rec60_flg = "Y";
            $hbr{ $item[1] } = { name => trim( $item[4] ) };
        }
        if ( $item[0] eq "120" ) {
            $model->misc->{120} = \@item;
            my $lot = $item[5];
...
sub computeTestTimeElapsedTime {
	my $touch_time = shift;
	my $curr_time = shift;
	my $prev_time = shift;
