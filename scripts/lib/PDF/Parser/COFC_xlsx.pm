# 2-May-2016 Eric	: copied from COFC_xls to handle the XLSX version
# 18-Aug-2017 Eric	: fixed script to handle misaligned specs against the measurement data
# 05-May-2018 Eric	: removed replacement or default values for N,MIN,MAX statistics.
...
sub readFile {
	...
	my $xlsx = $parser->parse($infile);
        my ($sh_h, $sh_l) = get_sheets($xlsx);
		...
		if ($h_dummy[0] =~ /DATE/i) {
			...
			if ($new_arr[1] =~ /(\d{4})\-(\d{2})\-(\d{2})/) # change yyyymmdd to yyyy/mm/dd
			...
			elsif ($new_arr[1] =~ /(\d{1,2})\/(\d{1,2})\/(\d{4})/)
                        {
                                ...
                                if (length($a) == 1) { $a = "0".$a; }
                                if (length($b) == 1) { $b = "0".$b; }
                                if ($facility =~ /SUNEDISON_US|WFRWRKS_TW|SILT_US/i)
                                {
                                        if ($a > 12) {
                                                $new_arr[1] = $c."-".$b."-".$a;
                                        }
	...
	my (@tnum_arr, @tnam_arr, @hlim_arr, @llim_arr, @unit_arr);
	my $lot_cnt = 0;	
	...
	for(my $iR = $start_row+1 ; defined $sh_l->{MaxRow} && $iR <= $sh_l->{MaxRow} ; $iR++)
	{
		...
		if ( defined $sh_l->{Cells}[$iR][$f{N}] ) 
		{
			...
			if (! $value || $value < 1)
			{
				WARN("N value not defined or less than 1");
				#$n = "1";
				$n = "";
			}
			else
...
sub get_sheets
{
        ...
        foreach my $oWkS (@{$oBook->{Worksheet}})
        {
                ...
                $l = $oWkS if ((!defined($l)) && ($oWkS->{Name} =~ /_l$|_lotstat$|lot/i)) ;
