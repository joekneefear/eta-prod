#!/usr/bin/env perl_db
# SVN $Id: fcs_stdf_advantest_IFF.pl 2274 2018-01-11 01:23:53Z dpower $
...
unless (
    GetOptions(
        \%hOptions, "OUT=s", "SITE=s", "LOC=s", "FACILITYFILE=s", "CONFIG=s", "FINALLOT", "RELLOT", "LOGFILE=s", "DEBUG", "TRACE", "V", "PPLOG"
    )
    )
{
...
$site = $hOptions{SITE};
unless ( grep { $_ eq $site } qw/casort_advan merel_advan mesort_advan hana_th_ft cpsort_advan amkor_tw_csp pmsort_advan pmft_advan utac_th_ft atec_ph_ft/ ) {
