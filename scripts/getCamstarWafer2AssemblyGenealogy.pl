...
use JSON::PP;
use Fcntl qw(:flock);
use Sys::Hostname;             # Added for hostname tracking
...
my $usageMsg = "Usage: $0 --source_db {source-DB} --source_warehouse {source-warehouse} --source_schema {source snowflake-db.schema} --start_hours num-hours --end_hours num-hours --logfile {log-file} --out_gen {wafer-genealogy-upload-dir} --archive_gen {wafer-genealogy-archive-dir} --out_trace {trace-upload-dir} --archive_trace {trace-archive-dir} --benchmark_log {jsonl-file} [--benchmark_include_non_archive] [--benchmark_db_dsn {oracle-dsn}] [--benchmark_db_user [{user}]] [--benchmark_db_pass [{password}]] --lock_file {lock-file} --pipeline_name {name} --pipeline_type {type}\n\nNote: If --benchmark_db_user is specified without a value, default credentials will be used (user: refdb)\n";
...
unless (GetOptions( \%hOptions, "SOURCE_DB=s","SOURCE_WAREHOUSE=s","SOURCE_SCHEMA=s", "START_HOURS=s", "END_HOURS=s", "OUT_GEN=s", "OUT_TRACE=s", "ARCHIVE_GEN=s", "ARCHIVE_TRACE=s", "LOGFILE=s", "BENCHMARK_LOG=s", "BENCHMARK_INCLUDE_NON_ARCHIVE!", "BENCHMARK_DB_DSN=s", "BENCHMARK_DB_USER:s", "BENCHMARK_DB_PASS:s", "LOCK_FILE=s", "PIPELINE_NAME=s", "PIPELINE_TYPE=s")){
    print($usageMsg);
    dpExit( 1, "invalid options" );
}
if (defined($hOptions{LOGFILE})) {
    my $logDir = dirname($hOptions{LOGFILE});
    if (!-d $logDir) {
        make_path($logDir);
    }
}
...
if (!defined($lockFile) || $lockFile eq "")
{
	...
	{
		$lockFile = "./log/getCamstarWafer2AssemblyGenealogy.lock";
	}
}
...
# Environment detection - mirrored from Python pipelines
my $detectedEnv = "prod";
my $hostname = lc(hostname() || $ENV{HOSTNAME} || $ENV{COMPUTERNAME} || "unknown");
...
my $pipelineName = $hOptions{PIPELINE_NAME} || "getCamstarWafer2AssemblyGenealogy";
...
# Calculate file type counts and rows
my %file_type_counts = ();
my %file_type_rows = ();
foreach my $fileName (@csv_files)
{
	# Extract file type from filename (e.g., a2w or w2a from filename.a2w.csv)
	if ($fileName =~ /\.([a-zA-Z0-9]+)\.csv$/)
	{
		my $file_type = $1;
...
# Record genealogy file stats separately
$file_type_counts{gen} = 1;
$file_type_rows{gen} = $genRowsWritten;
...
if ((defined($hOptions{BENCHMARK_LOG}) && length($hOptions{BENCHMARK_LOG}) > 0) || length($benchmarkDsn) > 0)
{
	...
	my @allGeneratedFiles;
	push @allGeneratedFiles, { path => File::Spec->rel2abs($genOutFile), rows => $genRowsWritten };
	push @allGeneratedFiles, { path => File::Spec->rel2abs($genArchFile), rows => $genRowsWritten };
	for (my $i=0; $i < scalar(@traceOutFiles); $i++)
	{
		my $fName = $csv_files[$i];
		my $rows = $file_type_row_counts{$fName} || 0;
		push @allGeneratedFiles, { path => File::Spec->rel2abs($traceOutFiles[$i]), rows => $rows };
		push @allGeneratedFiles, { path => File::Spec->rel2abs($traceArchFiles[$i]), rows => $rows };
	}
	# Calculate file type statistics and total rows written
	my %file_type_counts_stats = ();
	my %file_type_rows_stats = ();
	my $summedTraceRows = 0;
	
	# Add the genealogy file explicitly
	$file_type_counts_stats{"gen"} = 1;
	$file_type_rows_stats{"gen"} = $genRowsWritten || 0;

	foreach my $fileName (@csv_files)
	{
		# The filenames use custom extensions like .a2w, .w2a, etc. (often followed by .csv or .csv.gz)
		# Extract the core type (e.g. a2w from file.a2w.csv or file.a2w.csv.gz)
		my $file_type = "unknown";
		if ($fileName =~ /\.([a-zA-Z0-9]+)\.csv(?:\.gz)?$/)
		{
			$file_type = $1;
		}
		elsif ($fileName =~ /\.([a-zA-Z0-9]+)(?:\.gz)?$/)
		{
			$file_type = $1;
		}
		
		$file_type_counts_stats{$file_type} = ($file_type_counts_stats{$file_type} || 0) + 1;
		my $rowsInFile = $file_type_row_counts{$fileName} || 0;
		$file_type_rows_stats{$file_type} = ($file_type_rows_stats{$file_type} || 0) + $rowsInFile;
		$summedTraceRows += $rowsInFile;
	}
	
	my $totalRowsWritten = $genRowsWritten + $summedTraceRows;
	my $totalFiles = scalar(@csv_files) + 1;
	...
	my %stats = (
		...
		out_files => \@allGeneratedFiles,
		source_name => "Camstar Wafer to Assembly Genealogy",
		output_name => $outputFileGen || $outputFileTrace || basename($0)
	);
	...
	if ($@)
	{
		WARN("Benchmark JSONL logging failed (non-fatal): $@");
	}
	
	# Write to Oracle DB if credentials or DSN provided (best effort - don't fail if this errors)
	if (defined($hOptions{BENCHMARK_DB_USER}) || $hOptions{BENCHMARK_DB_DSN} || $ENV{BENCHMARK_DB_DSN})
	{
		eval {
		...
		if ($@)
		{
			WARN("Benchmark Oracle logging failed (non-fatal): $@");
		}
	}
}
...
sub writeBenchmarkToOracle()
{
	...
	my $dsn = $optionsRef->{BENCHMARK_DB_DSN} || $ENV{BENCHMARK_DB_DSN} || "exnqa-db.onsemi.com:1740/EXNQA.onsemi.com";
	...
	if (defined($optionsRef->{BENCHMARK_DB_USER}) && length($user) == 0)
	{
		$user = "refdb";
	...
	my %metadata = (
		source_name => $statsRef->{source_name} || "Camstar Wafer to Assembly Genealogy",
		output_name => $statsRef->{output_name} || "N/A",
	...
	# Serialize arrays to JSON strings for CLOB columns
	...
	my $sql = q{
		INSERT INTO pipeline_runs (
			...
			rows_extracted, rows_written, total_files, status, error_message,
			hostname, run_args, metadata, benchmark, out_files
		) VALUES (
	...
	eval {
		...
		$sth->bind_param(':archived_file', $statsRef->{archived_file} || "");
		$sth->bind_param(':output_file_gen', $statsRef->{output_file_gen} || "");
		$sth->bind_param(':output_files_gen', $outputFilesGenJson);
		$sth->bind_param(':output_file_trace', $statsRef->{output_file_trace} || "");
		$sth->bind_param(':output_files_trace', $outputFilesTraceJson);
		$sth->bind_param(':archived_gen_files', $archivedGenFilesJson);
		$sth->bind_param(':archived_trace_files', $archivedTraceFilesJson);
