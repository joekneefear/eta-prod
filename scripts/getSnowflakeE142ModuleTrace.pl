...
unless (GetOptions( \%hOptions, "SOURCE_ODBC=s", "SOURCE_WAREHOUSE=s", "SOURCE_SCHEMA=s", "FLOW=s", "GET_PRODUCT", "STAGE=s", "VIEW_NAME=s", "MODFILE=s", "MAX_HOURS=s", "START_HOURS=s", "END_HOURS=s", "OUT_TRACE=s", "LOGFILE=s", "BENCHMARK_LOG=s", "BENCHMARK_INCLUDE_NON_ARCHIVE!", "BENCHMARK_DB_DSN=s", "BENCHMARK_DB_USER:s", "BENCHMARK_DB_PASS:s", "PIPELINE_NAME=s", "PIPELINE_TYPE=s", "PROD_NOT_REGEXP=s", "LOCK_FILE=s")){
    print($usageMsg);
...
if ((defined $hOptions{BENCHMARK_LOG} && length($hOptions{BENCHMARK_LOG})) || (defined $benchmarkDsn && length($benchmarkDsn)))
{
		...
		foreach my $f (@csv_files)
		{
			...
			# Extract file type from filename extension (e.g. w2f from file.w2f.gz or file.w2f)
			my $file_type = "unknown";
			if ($base =~ /\.([a-zA-Z0-9]+)\.csv(?:\.gz)?$/)
			{
				$file_type = $1;
			}
			elsif ($base =~ /\.([a-zA-Z0-9]+)(?:\.gz)?$/)
			{
				$file_type = $1;
			}
	...
	my %bench = (
		...
		source_name => ($hOptions{VIEW_NAME} || "Snowflake E142 Module Trace"),
		output_name => $representative_output || basename($0)
	);
...
sub writeBenchmarkToOracle()
{
	...
	my %metadata = (
		source_name => $statsRef->{source_name} || $hOptions{VIEW_NAME} || "Snowflake E142 Module Trace",
	...
	my $sql = q{
		INSERT INTO pipeline_runs (
			...
			archived_file, rows_extracted, rows_written, total_files, out_files,
			output_file_gen, output_files_gen, output_file_trace, output_files_trace,
			archived_gen_files, archived_trace_files,
	...
	eval {
		...
		# New multi-file and gen/trace specific columns
		$sth->bind_param(':output_file_gen', "");
		$sth->bind_param(':output_files_gen', "[]");
		$sth->bind_param(':output_file_trace', $statsRef->{output_file} || "N/A");
		$sth->bind_param(':output_files_trace', $outputFilesTraceJson);
		$sth->bind_param(':archived_gen_files', "[]");
		$sth->bind_param(':archived_trace_files', "[]"); # E142 Perl currently doesn't separate archives
