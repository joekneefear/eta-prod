...
#
#      $Id: fcs_wkstrm.pl 1852 2016-10-03 15:12:48Z dpower $
      my($sVersionId) = ( split(' ', '$Revision: 1852 $') )[1];
	  my($VersionAndDate) = "
...
sub getPSVerLimitDate
{
   ...
   my $date = $parameter_sets{$psver_key}->{"YMS PSET Start Date"};

   my $limit_key = $program_name . "~~~" . $parameter_set_version;
   if (exists($limits{$limit_key}))
   {
      $date = $limits{$limit_key}->{DATE};
      #INFO("getPSVerLimitDate: Found date \"$date\" in hash ($program_name, $parameter_set_version)");
   }
   else
   {
      my $limit = new_limit;
      $limit->PROGRAM($program_name);
      $limit->REVISION($parameter_set_version);
      #INFO("Limit Date=\"$date\"");
      $limit->DATE($date);
      $limit->input_file(basename $ZIPFile);
      my $obj = {%$limit};
      my ($rc, $limit) = getRefdb->checkAndInsertLimitGetInfo($obj);
      if ( $rc == 1 )
      {
         # Parameter set limits not registered. Use date from parameter set.
         #INFO("getPSVerLimitDate: New Limit in REFDB, date \"$date\" ($program_name, $parameter_set_version)");
      }
      else
      {
         # Get date from database.
         $date = $limit->{DATE};
         #INFO("getPSVerLimitDate: Existing Limit in REFDB, date \"$date\" ($program_name, $parameter_set_version)");
      }
...
#			my($sourcelot,$lotowner,$lotclass,$product) = GetMetaByLot($lot);
			...
			for($i=1;$i<=$#vs+1;$i++){
				...
				# Get Date limit was first seen (or current date if not seen).
				# Use this as the parameter set revision date.
				my $psver_date = getPSVerLimitDate($progName, 
				                                   $phistinfo{$holder}{'parameter_set_id'},
					   			   $phistinfo{$holder}{'parameter_set_version'});
...
sub OutputMetOffLine{
	...
	for($i=1;$i<=$#vs+1;$i++){
		...
		my $offline = join("|",(FormatField($phistinfo{$holder}{'date_time'}),FormatField($phistinfo{$holder}{'facility'}),FormatField($entinfo{$entkey})
							...
							,FormatField($vs[$i-1]), FormatField($psver_date)
