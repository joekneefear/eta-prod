...
# print "Location:http://usaz15ls081.onsemi.com/main.html.new";
#
# exit 0 if fork();
# close(STDIN);
# close(STDOUT);
    ...
    function reset_form()
    {
        if (confirm("Would you like to erase your entries and reset the form?") == true)
        {
            frmMain.action = "?Reset=Yes";
						//document.getElementById("btnReset").action = "?Reset=Yes";
            frmMain.submit();
...
#print '<div id="loading">  <img id="loading-image" src="https://media.giphy.com/media/jAYUbVXgESSti/giphy.gif" alt="Loading..." /></div>';
...
if (keys %pa == 0 ||keys %envs == 0)
{
	...
	#}
  #print '<div > <img id="loading-image" src="../images/spinner.gif" alt="Searching lot in archive..." /> </div>';
	#############
	# START FORM
	#############
		...
		print "<td align=right>",
		      ...
		      textfield({-name =>"txtUsername",
			         -size =>40,
				 -title=>"Please type your name so that IT CIM YQS DataIntegration <IT-CIM-YQS-DataIntegration@onsemi.com> can support your request in case manual intervention is necessary.",
				 -value=>$username}),
		      "</td>",
		      "</tr>",
		      "</table>";
		...
		for(my $i=1; $i<=$max_rows; $i++)
		{
			...
			my $color  = ($result =~ /Obsoleted/i) ? "red" : "black";
			print "<td>",
			      input({-type    =>"text",
