...
# 08-10-2011  Ben Rommel Kho  Adopted .TP filenaming convention.
...
############
my $file        = "";
my $subject     = "PMFFET PRN CONVERTER";
%testplan  	= ();
%good_bins	= ();
#$FTFET_REF 	= "$ENV{ENV_DB_SCRIPT}/ref_ftstfet.txt";
$FTFET_REF     = "/home/dpower/project/scripts/stdf_perl/ref_ftstfet.txt";
$polarity_flag  = "";
$tp_name	= "";
...
########################
print "$file".'.TP'       if $mft_flag==0;
print "\ntp=$file".'.TP'  if $mft_flag==1;
...
sub parse_prn
{
	...
	while(<FH>)
	{
		...
		if ($_=~/TEST\:/ && $tp_name eq "")
		{
			#====================================================================================
                        #          1         2         3         4         5         6         7         8
                        #012345678901234567890123456789012345678901234567890123456789012345678901234567890
			#TEST:C0606A.RUN      RUN FILE CREATED:   20 AUGUST 2001
			#====================================================================================
			$tp_name    = uc(substr $_, 5, 16);
			...
			if ($hbin_num=~/\b\d{1,2}R\b/)
			{
				foreach (@test_nums)
				{
					...
					if($_ ne "")
					{
						...
						$testplan{$_}{HBIN_NAM} = "BIN".$hbin_num;
					}
				}
			}
...
sub detect_channel
{
        ...
        if ( $polarity_flag eq "")
        {
		$error_msg = "cannot detect channel. please check testpan file <$file>";
                &send_email($subject, $error_msg);
		print "$error_msg\n";
                exit 1;
        }

	#print "CHANNEL: $polarity_flag\n";
}
...
sub abs_to_signed_limits
{
	foreach (sort {$a <=> $b} keys %testplan)
		...
		{
			$error_msg = "$testname not in test polarity ref file. Please update";
			&send_email($subject, $error_msg);
			print "$error_msg\n";
			exit 1;
		}
...
sub create_tp
{
	...
	if($tp_name eq "")
	{
		### FOR MODIFIED FILENAME DURING DISPATCHING ###
		@pathfile = split /\//,$file;
		$tp_name  = $pathfile[-1];	
		if ($tp_name =~/CPFTSTFET/i)
		{
			($tp_name,) = split /_/,$tp_name;
		}
		else
		{
			($tp_name,) = split /\./,$tp_name;
		...
		$tp_name    = uc $tp_name;
		print "Invalid PRN header format. Setting PRN filename as the testplan name...\n";
	}

	my $DateTime         = `date '+%m%d%y%H%M%S'`;
        chomp($DateTime);
	#$tp_name = "FT_".$tp_name;
        open FH, ">${file}.TP" or die "error msg: $!";
