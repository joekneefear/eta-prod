...
# 2021-Apr-15 glory     :not to hardcode directory and change it to Colo location.
...
if ($infile =~ /DATA\.XLS$|DATA\.XLS_MD5.+/i) {
	system "/bin/mv -f \'${infile}\' ${data_dir}/${move_file}";
        $status = (-e "${data_dir}/${move_file}") ? "Successful" : "Failed";
}
elsif ($infile =~ /SUM\.XLSX$|SUM\.XLSX_MD5.+|SUM\.XLS$|SUM\.XLS_MD5.+/i ) {
