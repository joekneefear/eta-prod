...
This script will preprocess/enrich JND LEH file.
...
#     for arg in log_args:
#         if arg in sys.argv:
#             log_file_index = sys.argv.index(arg) + 1
#             if log_file_index < len(sys.argv):
#                 log_file = sys.argv[log_file_index]
#                 break  # exit the loop once we've found a log file argument
#     return log_file
def initialize_log_file():
    script_name = os.path.basename(sys.argv[0])  # get the script filename
    log_file_name = os.path.splitext(script_name)[0] + '.log'  # remove extension and add .log
...
def main():
    ...
    iff_instance.write_dict_line_list()
