...
def main():
    ...
    lot_metadata_location = yaml_data['SPEC']['lot_metadata_location']
    ...
    map_spec_sxml_parser = SpecXmlParser(input_file)
    lot = map_spec_sxml_parser.get_lot_value()
    ...
    spec_sxml_enricher = SpecXmlEnricher(input_file, refdb_api_client, ws_urls, default_onscribe_metadata, default_onlot_metadata)
    enriched_map_spec_xml, new_waferids = spec_sxml_enricher.enrich(lot, jnd_motherlot, jnd_sourceLot, lot_metadata_file_fullpath, waferids)
    # print(enriched_kdf_xml)
    sxml_instance = SXML(writer=writer_instance, sxml=enriched_map_spec_xml)
