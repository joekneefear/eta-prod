...
from lib.Enricher.KDFXmlEnricher import KDFXmlEnricher
from lib.Parser.KDFXmlParser import KDFXmlParser
...
def main():
    ...
    lot_metadata_location = yaml_data['KDF']['lot_metadata_location']
    ...
    default_jnd_fab = 'JND:AIZU2 FAB (PTI)'
    ...
    kdf_sxml_parser = KDFXmlParser(input_file)
    lot = kdf_sxml_parser.get_lot_value()
    ...
    if Util.check_file_exists(lot_metadata_file_fullpath):
        ...
        Log.INFO(f"Lot={lot} extracted from the PCM filename")
        if lot:
            jnd_motherlot, jnd_sourceLot = JndUtil.get_jnd_lot_mother_lot_source_lot_not_refdb(lot)
            lot_metadata_file = f"{jnd_motherlot}.lot"
            lot_metadata_file_fullpath = os.path.join(lot_metadata_location, lot_metadata_file)
    ...
    kdf_sxml_enricher = KDFXmlEnricher(input_file, refdb_api_client, ws_urls, default_onscribe_metadata, default_onlot_metadata)
    enriched_kdf_xml, new_waferids = kdf_sxml_enricher.enrich(lot, jnd_motherlot, jnd_sourceLot, lot_metadata, last_lot_metadata, waferids)
    if not is_lot_metadata:
        Log.WARN(f"No lot metadata available cant get Technology and LotType, to SANDBOX")
        writer_instance.noMeta = True
    # print(enriched_kdf_xml)
    sxml_instance = SXML(writer=writer_instance, sxml=enriched_kdf_xml)
