...
class Monitor:
    ...
    def is_log_file_active(self, log_file, timeout):
        timeout = 3600  # 1 hour
        now = time.time()
...
@app.get("/status")
def get_status():
    return JSONResponse(content=monitor_instance.get_status())
