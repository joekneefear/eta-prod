...
class E142ExtractionManager:
    ...
    def build_command(self, facility: str, stage: str, 
                     ...
                     max_hours: Optional[int] = None) -> List[str]:
        ...
        cmd = [
            ...
            '--out_trace', fac_config['out_trace'],
            '--logfile', logfile,
            '--pipeline_name', f"E142_{facility}_{stage}",
        ]
    ...
    def run_extraction(self, facility: str, stage: str,
                      modfile: Optional[str] = None,
                      start_date: Optional[str] = None,
                      max_hours: Optional[int] = None) -> Tuple[bool, str]:
        ...
        logfile = None  # Initialize logfile
        modfile_path = None  # Initialize modfile_path
        
        try:
            cmd, modfile_path, logfile = self.build_command(
                facility, stage, modfile, start_date, None, max_hours
            )
            ...
            if start_date and not modfile and modfile_path and Path(modfile_path).exists():
    ...
    def run_manual_mode(self, facility: str, stage: str, 
        ...
        success, _ = self.run_extraction(facility, stage, modfile, None, max_hours)
        sys.exit(0 if success else 1)
    ...
    def run_historical_mode(self, facility: str, stage: str,
        ...
        for i, date in enumerate(dates, 1):
            ...
            success, logfile = self.run_extraction(
                facility, stage, None, date_str, max_hours
            )
...
Examples:
  ...
  python e142_extraction_manager.py all --stage WAFER
    ...
    if args.mode == 'cron':
        ...
        manager.run_manual_mode(args.facility, args.stage, args.modfile, args.max_hours)
