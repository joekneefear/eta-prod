...
class KDFXmlEnricher:
    ...
    def enrich(self, lot, mother_lot, sourceLot, lot_metadata, last_lot_metadata, waferids):
        ...
        # lot_metadata, last_lot_metadata = Util.load_jnd_lot_metadata(lot_metadta_file)
        # # print(f"LOTM={lot_metadata}")
        jnd_mother_lot = mother_lot
        jnd_sourceLot = sourceLot
        ...
        for line in self._txt_content.splitlines():  # Split content into lines
            ...
            if stripped_line.startswith("<File"):  # Check if the line starts with <File
                match = re.search(r'FileName="([^"]*)"', line)
                if match:
                    value = match.group(1)
                    value = re.sub(r'^.*/', '', value)  # Remove everything before the last '/'
                    value = re.sub(r'\.[^\.]*$', '', value)  # Remove everything after the last '.'
                    if value:
                        modified_line = re.sub(r'>$', f' FileNameWoS="{value}">', modified_line)
                        # print(f"Added FileNameWoS to line: {modified_line}")
            ...
            if stripped_line.startswith("<Lot "):  # Check if the line starts with <Lot
                ...
                if lot in lot_metadata:
                    # Update/add required attributes from lot_metadata
                    ...
                    for key in ['TPNO', 'AccountCode', 'MBNO', 'Process', 'Technology', 'Fab', 'SourceLot', 'ParentLot', 'LotType']:
                        attributes[key] = lot_metadata[lot].get(key, 'NA')
                else:
                    # Update/add required attributes from last_lot_metadata
                    attributes['LotId'] = lot
                    ...
                    for key in ['TPNO', 'AccountCode', 'MBNO', 'Process', 'Technology', 'Fab', 'SourceLot', 'ParentLot', 'LotType']:
                        attributes[key] = last_lot_metadata.get(key, 'NA')
                ...
                # Process Lmt and Wdf
                for tag in ['Lmt', 'Wdf']:
                    match = re.search(rf'{tag}="([^"]*)"', line)
                    if match:
                        value = match.group(1)
                        value = re.sub(r'^.*/', '', value)  # Remove everything before the last '/'
                        value = re.sub(r'\.[^\.]*$', '', value)  # Remove everything after the last '.'
                        if value:
                            modified_line = re.sub(r'>$', f' {tag}WoS="{value}">', modified_line)

            # Process wafer information
            if stripped_line.startswith("<Wafer"):  # Check if the line starts with <Wafer
                # print(f"Processing Wafer line: {line}")
                slot_match = re.search(r'Slot="([^"]*)"', line)
                if slot_match:
                    slot = slot_match.group(1)
                    # slot = slot.zfill(2)
                    slot = Util.format_wafer_number(slot)
                    lot_wafer_key = f"{lot}_{slot}"
                    ...
                    onscribe_metadata = Util.replace_dict_value(onscribe_metadata, 'waferNum', slot)
                    ...
                    onscribe_metadata = Util.update_wafer_id(onscribe_metadata, jnd_sourceLot, slot)
                    waferid = onscribe_metadata.get('waferId', 'NA')
                    if Util.looks_like_number(slot):
                        waferid = JndUtil.get_waferid_scribe_file(lot, slot, waferids)
                        if waferid == 'NA':
                            # onscribe_ws_url = self.ws_urls.get('onscribe', '')
                            onscribe_metadata, new_lot_wafer_key = JndUtil.get_jnd_onscribe_metadata(self.refdb_api_client, self.ws_urls, jnd_sourceLot, waferids, lot, slot, onscribe_metadata, onlot_metadata)
                            waferid = onscribe_metadata.get('waferId', 'NA')
        ...
        # Return the list of enriched lines
