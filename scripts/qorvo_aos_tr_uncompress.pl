...
if ( $#ARGV < 0 ) {
        print "Usage: $0 <FILENAME> --OUT1 <DESTINATION_DIR1> \n";
        exit 1;
}
unless (
        GetOptions(
                \%hOptions,  "OUT1=s", "LOGFILE=s", "DEBUG", "TRACE"
        )
)
{
...
=pod
		...
		#}
=cut
		moveFileByExt($ext_file,$des1_dir);
...
=cut
	...
	moveFileByExt($dcom_file,$des1_dir);
...
sub moveFileByExt {
	...
	#}
		...
		move($fle, $mv_fname);
		my $status = ( -e $mv_fname) ? "Successful" : "Fail";
		INFO("Moving $fname to $des3_dir : $status");		
	}
