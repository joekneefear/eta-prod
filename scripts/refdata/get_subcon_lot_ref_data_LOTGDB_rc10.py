...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--lock_file", help="Lock file path (optional, default: ./log/<pipeline_name>.lock)")
...
def log_benchmark_to_oracle(stats: dict, dsn: str, user: str = None, password: str = None) -> None:
    ...
    if not user:
        ...
        logging.info(f"Using default benchmark database credentials (user: {user})")
    ...
    try:
        ...
        # Prepare metadata JSON (empty for this script, can be extended)
        metadata = {}
        ...
        logging.error(f"Failed to insert benchmark into Oracle: {ex}")
        if conn:
            try:
                conn.rollback()
            except Exception:
...
def main():
    ...
    try:
        ...
        if not lock_file:
            safe_pipeline = pipeline_info["pipeline_name"].replace("/", "_").replace("\\", "_")
        ...
        stats = {
            ...
            "total_files": 1,  # Single output file per get_subcon_lot_ref_data run
