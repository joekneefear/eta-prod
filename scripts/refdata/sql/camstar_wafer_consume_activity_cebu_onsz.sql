...
SELECT
    ...
    am.FabLotNumber,
    MAX(SUBSTRING(hd.MaterialLotName, 1, CASE WHEN PATINDEX('%-%', hd.MaterialLotName) = 0 THEN 40 ELSE PATINDEX('%-%', hd.MaterialLotName) - 1 END)) AS MaterialLotID,
...
WHERE SUBSTRING(hd.MaterialLotName, 1, CASE WHEN PATINDEX('%-%', hd.MaterialLotName) = 0 THEN 40 ELSE PATINDEX('%-%', hd.MaterialLotName) - 1 END) IS NOT NULL
GROUP BY
    hm.ContainerName,
    CASE WHEN hm.ownername LIKE 'ON%' AND LEN(hm.OwnerName) = 4 THEN SUBSTRING(hm.OwnerName, 3, 2) ELSE hm.OwnerName END,
    hm.ProductName,
    hm.historyid,
    hm.Qty,
